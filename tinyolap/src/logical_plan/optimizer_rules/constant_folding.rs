...
//!
//! Evaluates `BinaryOp` expressions where both operands are literals, folding them
//! to a single literal at plan time instead of at execution time.
//!
//! For example:
//!   - Int comparisons: `1 = 1` → `true`, `1 = 2` → `false`
//! Does not fold float comparisons or string comparisons — not needed yet.

use crate::logical_plan::{
    logical_operators::{BinaryOp, LiteralValue, LogicalExpr, LogicalPlan},
    ...
    match expr {
        LogicalExpr::BinaryOp { left, op, right } => {
            ...
            match (&left, &op, &right) {
                ...
                (
                    LogicalExpr::Literal(LiteralValue::Int(a)),
                    _,
                    LogicalExpr::Literal(LiteralValue::Int(b)),
                ) => {
                    let result = match op {
                        BinaryOp::Eq => a == b,
                        BinaryOp::NotEq => a != b,
                ...
                (LogicalExpr::Literal(LiteralValue::Bool(false)), BinaryOp::And, _) => {
                    LogicalExpr::Literal(LiteralValue::Bool(false))
                } // false AND anything = false
                (LogicalExpr::Literal(LiteralValue::Bool(true)), BinaryOp::Or, _) => {
