...
fn multi_part() {
    ...
    assert_eq!(rows(&q("SELECT ts FROM wide")), n, "1col");
    let two = q("SELECT ts, f64_c FROM wide");
    assert_eq!(two[0].num_columns(), 2, "2col cols");
    assert_eq!(rows(&two), n, "2col rows");
    let all = q("SELECT * FROM wide");
    assert_eq!(all[0].num_columns(), 12, "all cols");
    assert_eq!(rows(&all), n, "all rows");
}
