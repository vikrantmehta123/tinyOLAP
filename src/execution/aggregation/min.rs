    ...
    T::Native: PartialOrd
{
    ...
    ) -> Result<(), ExecutionError>
    {
        if self.has_value.len() < num_groups {
            ...
            self.running_minimums.resize(num_groups, T::Native::default());
        }
        ...
        for (i, &gi) in group_indices.iter().enumerate() {
            ...
            if value.is_lt(self.running_minimums[gi as usize]) {
                self.running_minimums[gi as usize] = value;
            }
        }
    ...
    fn finalize(&mut self) -> ArrayRef {
        Arc::new(PrimitiveArray::<T>::from_iter_values(
            std::mem::take(&mut self.running_minimums),
        ))
    }
